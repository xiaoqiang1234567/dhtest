...
u_char ip_listen_flag = 1;
u_char listen_time_flag = 0;
...
void print_help(char *cmd)
{
	...
	fprintf(stdout, "  dhtest version 1.1-rtsp\n");
}
...
int main(int argc, char *argv[])
{
   ...
   // build_option61_vci();	/* Option61 - VCI  */
	...
	while(dhcp_offer_state != DHCP_OFFR_RCVD) {
        ...
        fprintf(stdout, "6dhcp_release_flag %d\n",dhcp_offer_state);
		if(timeout) {
			time_now = time(NULL);
	...
	if(!nagios_flag && ip_listen_flag) {
		if(!json_flag) {
			...
			fprintf(stdout, "IP address: %s, Listen timeout: %d seconds  %d tv_sec\n", get_ip_str(htonl(ip_address)), listen_timeout,tval_listen.tv_sec);
		} else {
			if(!json_first) {
				fprintf(stdout, ",");
			} else {
